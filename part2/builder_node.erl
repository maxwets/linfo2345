...
%%%% INIT FUNCTIONS %%%%
init(PID_List, Nodes) ->
	TransactionList = utils:readlines("transactions.csv"),
...
builder_loop(PID_List, Nodes, TxRemaining, BlockChain) ->
	...
	MainProposer = lists:nth(1, Nodes#nodes.proposers),
			...
			receive
				{continue, From} ->
					io:format("[builder] ~p received continue from ~p~n", [self(), From]),
					if
						length(TxRemaining) == 0 ->
							io:format("[builder] ~p has finished, sending stop to all other nodes~n", [self()]),
...
builder_change_epoch_loop(PID_List, Nodes, TxRemaining, BlockChain) ->
	...
	receive
		{proposers, List, From} ->
			io:format("[builder_change_epoch_loop] ~p received ~w as the new list of proposers from ~p~n", [self(), List, From]),
			if
				...
				true ->
					io:format("[builder_change_epoch_loop] ~p received ~w from ~p while the previous main proposer is ~p~n", [self(), List, From, PreviousMainProposer]),
					builder_change_epoch_loop(PID_List, Nodes, TxRemaining, BlockChain)
			end
	end.
...
buildNewBlock(TxRemaining, BlockChain) ->
	...
	SizeOfBlock = (rand:uniform(9) rem N) + 1,
